...
| TCK JSON-RPC MUST tests against the repository-owned SUT | evidence with waivers | Fixed TCK `5996b79f`: pytest `81 passed, 154 skipped, 30 deselected`, compatibility registry `67 PASS / 25 SKIPPED / 37 NOT TESTED`; report is written by [`tests/conformance/run-tck.sh`](../../tests/conformance/run-tck.sh) |
...
| Owned SUT HTTP+JSON MUST tests | not configured | The owned fixture advertises JSON-RPC+SSE only; HTTP+JSON remains a separate implementation gate |
...
The repository-owned SUT covers deterministic direct Message, Task, text/raw/URL/data Artifact, chunked SSE, `INPUT_REQUIRED`, cancellation, resubscription, rejection, version validation, history limits, and JSON-RPC ErrorInfo/content-type behavior. The selected JSON-RPC/SSE MUST run exits successfully, but the compatibility registry still records untested bindings/capabilities and the external TCK remains pinned to an older embedded protocol commit. These results are evidence with explicit waivers, not a claim of complete multi-binding conformance.
